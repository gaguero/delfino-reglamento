...
export async function POST(
  ...
  try {
    // Get the "Jurisprudencia" tipo anotacion
    const tipoJuris = await prisma.tipoAnotacion.findUnique({
      where: { nombre: 'Jurisprudencia' }
    })

    if (!tipoJuris) {
      return NextResponse.json({ error: 'TipoAnotacion "Jurisprudencia" not found' }, { status: 404 })
    }
